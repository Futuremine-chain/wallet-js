...
function ubAddress(buffer, version) {
    let ub = Buffer.concat([Buffer.from(version), ripemd160(sha256(buffer))]);
    let checkSum = sha256(sha256(ub)).slice(0, 4);
    return base58.encode(Buffer.concat([ub, checkSum]));
}
